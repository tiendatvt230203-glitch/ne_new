-- Config 30: bonding + weighted WAN. Policy 421 UDP + 422 TCP — both encrypt_l2 (0x88B5 on wire).
...
(30, 'wan_enp7s0_enp8s0_70_30', 1, 'Bonding: 421 UDP L2 + 422 TCP L2 GCM (all L2).');
...
SELECT 422, p.id, 20, 'encrypt_l2', 'tcp', 'gcm', 128, 12,
       '00112233445566778899aabbccddeeff'
FROM xdp_profiles p WHERE p.config_id = 30 AND p.profile_name = 'wan_enp7s0_enp8s0_70_30';
