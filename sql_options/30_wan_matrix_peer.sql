-- Peer / site B (WAN next-hop .1): same policy ids (420-423) and keys as 30_wan_matrix.sql.
-- Rows list src = local hosts on this side (180.x / 182.x), dst = remote (9.x / 10.x) for one readable convention.
-- Selection still matches both directions in code; WAN decrypt keys come from policy id inside the ciphertext.
...
-- CRYPTO_POLICY_MATCH_IP_ONLY: use ANY for ports.
...
(422, '192.168.182.2/32', 'ANY', '192.168.10.2/32', 'ANY'),
(423, '192.168.182.2/32', 'ANY', '192.168.9.2/32',  'ANY'),
(420, '192.168.180.2/32', 'ANY', '192.168.10.2/32', 'ANY');
