...
static int load_profiles_and_policies(struct app_config *cfg, PGconn *conn, int config_id) {
    ...
    PGresult *    res = PQexecParams(conn,
        "SELECT id, profile_name, enabled "
    ...
    int col_name = PQfnumber(res, "profile_name");
    int col_en = PQfnumber(res, "enabled");
    if (col_id < 0 || col_name < 0 || col_en < 0) {
        fprintf(stderr, "[DB] xdp_profiles row missing expected columns\n");
        PQclear(res);
        return -1;
    }
    ...
    for (int i = 0; i < nprof; i++) {
        ...
        memset(p, 0, sizeof(*p));
        p->id = atoi(PQgetvalue(res, i, col_id));
        strncpy(p->name, PQgetvalue(res, i, col_name), sizeof(p->name) - 1);
        p->enabled = atoi(PQgetvalue(res, i, col_en));
        cfg->profile_count++;
    }
    PQclear(res);
...
static int db_load_local_configs(PGconn *conn, struct app_config *cfg, int config_id)
{
    ...
    PGresult *res = PQexecParams(conn,
        "SELECT ifname FROM xdp_local_configs WHERE config_id = $1 ORDER BY id",
        1, NULL, params, NULL, NULL, 0);
